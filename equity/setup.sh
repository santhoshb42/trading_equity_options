...
setup_systemd_service() {
    log "🚫 Equity trading is retired; blocking systemd setup"

    if [ "$EUID" -eq 0 ]; then
        systemctl stop equity-bot.service equity-bot-watchdog.timer equity-bot-watchdog.service 2>/dev/null || true
        systemctl disable equity-bot.service equity-bot-watchdog.timer equity-bot-watchdog.service 2>/dev/null || true
        rm -f /etc/systemd/system/equity-bot.service \
            /etc/systemd/system/equity-bot-watchdog.service \
            /etc/systemd/system/equity-bot-watchdog.timer
        ln -sfn /dev/null /etc/systemd/system/equity-bot.service
        ln -sfn /dev/null /etc/systemd/system/equity-bot-watchdog.service
        ln -sfn /dev/null /etc/systemd/system/equity-bot-watchdog.timer
        systemctl daemon-reload
        log "✅ Equity systemd units are masked and cannot be started"
    else
        log "⚠️  Equity trading is retired; no systemd changes applied without root"
    fi
}
...
start_services() {
    log "🚫 Equity trading is retired; refusing to start services"
    return 1
}

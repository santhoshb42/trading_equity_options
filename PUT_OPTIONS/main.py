...
# Parse --mode BEFORE any optcode imports so optconfig.py sees BOT_MODE in environ
_arg_parser = argparse.ArgumentParser(add_help=False)
_arg_parser.add_argument('--mode', choices=['OTM', 'ITM'], default='OTM',
                         help='Bot mode: OTM (port 8082) or ITM (port 8083)')
_mode_args, _ = _arg_parser.parse_known_args()
os.environ['BOT_MODE'] = _mode_args.mode.upper()

# Add parent directory to path
sys.path.insert(0, str(Path(__file__).parent))

# Add trading/ and trading/tools/ for shared modules (alert_system, eod_backup_handler)
sys.path.insert(0, str(Path(__file__).parent.parent))
sys.path.insert(0, str(Path(__file__).parent.parent / 'tools'))

from optcode.optconfig import (
    WebhookConfig, OptionsTradingConfig, DevConfig, MonitoringConfig,
    get_optconfig_summary, validate_optconfig, OptionsCapitalConfig, DATA_DIR
)
...
# Aggregated live data (merges OTM+ITM into PUT_OPTIONS/live_data.json)
try:
    from aggregate_live_data import start_aggregator, stop_aggregator
    HAS_AGGREGATOR = True
except ImportError:
...
class OptionsTradingBot:
    ...
    def initialize(self):
        ...
        # Start live data aggregator (merges OTM+ITM → PUT_OPTIONS/live_data.json)
        if HAS_AGGREGATOR:
            start_aggregator(interval=10)
            logger.info("BOT_INIT: PE_AGGREGATOR_STARTED")
    ...
    def _cleanup_stale_positions(self):
        ...
        try:
            ...
            archive_file = DATA_DIR / "option_positions_archive.json"
            existing_archive = []
            ...
            while self.running:
                try:
                    ...
                    if not ltps_refreshed:
                        ...
                        if HAS_LIVE_DATA and self.live_data_tracker:
                            try:
                                ...
                                pass
                        # Idle: no positions and market closed — sleep longer to reduce CPU burn
                        idle_sleep = current_interval if len(self.monitor.positions) > 0 else 30
                    ...
                    if realtime_fade_exits:
                        ...
                        for pos in realtime_fade_exits:
                            print(f"   📉 Realtime fade exit: {pos['symbol']} PnL: ₹{pos['pnl']:.2f}")
                            logger.warning(f"POSITION_MONITOR: REALTIME_PREMIUM_FADE | {pos['symbol']} | PnL=₹{pos['pnl']:.2f} | reason={pos.get('exit_reason', 'REALTIME_PREMIUM_FADE')}")
                            if self.alert_manager:
                                try:
                                    self.alert_manager.alert_position_closed(
                                        bot_type='options',
                                        position={
                                            ...
                                            'pnl': pos['pnl'],
                                            'pnl_percent': pos.get('pnl_percent', 0),
                                            'reason': pos.get('exit_reason', 'Realtime premium fade')
                                        }
                                    )
                                except Exception as e:
                                    logger.warning(f"POSITION_MONITOR: REALTIME_PREMIUM_FADE_ALERT_FAILED | {str(e)}")
...
if __name__ == "__main__":
    ...
    _bot_mode = os.environ.get('BOT_MODE', 'OTM')        # e.g. "OTM"
    _mode_dir  = script_dir / _bot_mode.upper()            # PUT_OPTIONS/OTM/ or PUT_OPTIONS/ITM/
    ...
    def is_our_bot_running(pid: int) -> bool:
        """Return True only if PID exists AND its cmdline belongs to PUT_OPTIONS/main.py."""
        try:
            os.kill(pid, 0)
        except OSError:
        ...
        try:
            ...
            return 'PUT_OPTIONS/main.py' in cmdline
        except Exception:

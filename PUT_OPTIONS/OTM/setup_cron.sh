#!/bin/bash

# Cron Setup Script for PUT Options Bot
# Automatically starts bot at 9 AM and triggers EOD learning at 3:30 PM
# This ensures EOD learning never misses due to late bot starts
# SEPARATE FROM CE BOT - Uses port 8082 and process name "put_optbot"

BOT_DIR="/root/santhosh/trading/put_options"
PYTHON="/usr/bin/python3"

CRON_REBOOT='45 8 * * 1-5 /sbin/shutdown -r +5 # System reboot at 8:45 AM (5 min delay)'
LEGACY_CRON_REBOOT='45 8 * * 1-5 /sbin/shutdown -r +5'
CRON_INSTRUMENT='50 8 * * 1-5 cd /root/santhosh/trading/put_options && python3 tools/fetch_nfo_instruments.py >> /tmp/put_optbot_instrument_refresh.log 2>&1 # PUT instruments refresh'
CRON_ROUTER='55 8 * * 1-5 systemctl is-active --quiet webhook-router.service || systemctl start webhook-router.service >> /tmp/router_health.log 2>&1 # Ensure router is running'
LEGACY_CRON_ROUTER='55 8 * * 1-5 sudo systemctl is-active webhook-router || sudo systemctl start webhook-router >> /tmp/router_health.log 2>&1'
CRON_START='0 9 * * 1-5 systemctl start put_optbot.service >> /tmp/put_optbot_startup.log 2>&1 # PUT options bot auto-start (port 8082, separate from CE)'
CRON_HEALTH='*/30 9-16 * * 1-5 systemctl is-active --quiet put_optbot.service || systemctl start put_optbot.service >> /tmp/put_optbot_health.log 2>&1 # PUT options bot health check'
LEGACY_CRON_START='0 9 * * 1-5 cd /root/santhosh/trading/put_options && python3 main.py >> /tmp/put_optbot_startup.log 2>&1 # PUT options bot auto-start (port 8082, separate from CE)'
LEGACY_CRON_HEALTH_DIRECT="*/30 9-16 * * 1-5 pgrep -f '/root/santhosh/trading/put_options/main.py' > /dev/null || cd /root/santhosh/trading/put_options && python3 main.py >> /tmp/put_optbot_health.log 2>&1 # PUT options bot health check"
CRON_CLEANUP='0 0 * * 0 find /root/santhosh/trading/put_options/logs -type d -mtime +7 -exec rm -rf {} \; 2>/dev/null # PUT options cleanup old logs'
CRON_EXPORT='0 16 * * 1-5 cd /root/santhosh/trading/put_options && python3 -c "from optcode.trade_logger import get_trade_logger; logger = get_trade_logger(); logger.export_analysis()" >> /tmp/put_optbot_eod_export.log 2>&1 # PUT EOD stats export'

echo "=========================================="
echo "📅 Setting up Automatic PUT Options Trading Schedule"
echo "=========================================="
echo ""

# Backup current crontab
CRONTAB_BACKUP="/tmp/crontab_backup_put_$(date +%s).txt"
crontab -l > "$CRONTAB_BACKUP" 2>/dev/null || true

echo "Current crontab backed up to: $CRONTAB_BACKUP"
echo ""

# Create a temporary crontab file
TEMP_CRON="/tmp/new_crontab_put_options.txt"

# Copy existing crontab
crontab -l > "$TEMP_CRON" 2>/dev/null || true

remove_line() {
	local line="$1"
	local tmp_file
	tmp_file="${TEMP_CRON}.tmp"
	grep -Fvx -- "$line" "$TEMP_CRON" > "$tmp_file" 2>/dev/null || true
	mv "$tmp_file" "$TEMP_CRON"
}

# Remove only this bot's managed lines so other bots keep their cron jobs.
remove_line "$CRON_REBOOT"
remove_line "$LEGACY_CRON_REBOOT"
remove_line "$CRON_INSTRUMENT"
remove_line "$CRON_ROUTER"
remove_line "$LEGACY_CRON_ROUTER"
remove_line "$CRON_START"
remove_line "$CRON_HEALTH"
remove_line "$LEGACY_CRON_START"
remove_line "$LEGACY_CRON_HEALTH_DIRECT"
remove_line "$CRON_CLEANUP"
remove_line "$CRON_EXPORT"

echo "Adding new cron jobs for PUT Options Bot..."
echo ""

# Add reboot at 8:45 AM (5 minute delay, finishes by 8:50)
echo "$CRON_REBOOT" >> "$TEMP_CRON"
echo "✅ Added: System reboot at 08:45 AM with 5-min delay (Mon-Fri)"

# Add instrument.json refresh at 8:50 AM (before market open) - PUT specific
echo "$CRON_INSTRUMENT" >> "$TEMP_CRON"
echo "✅ Added: Daily instrument.json refresh for PUT at 08:50 AM (Mon-Fri)"

# Add webhook router check at 8:55 AM (ensure it's running)
echo "$CRON_ROUTER" >> "$TEMP_CRON"
echo "✅ Added: Webhook router health check at 08:55 AM (Mon-Fri)"

# Add PUT bot startup at 9 AM (Monday-Friday) - on port 8082, separate process
echo "$CRON_START" >> "$TEMP_CRON"
echo "✅ Added: Auto-start PUT options bot at 09:00 AM on port 8082 (Mon-Fri)"

# Add bot health check every 30 minutes during trading hours (9 AM to 4 PM)
echo "$CRON_HEALTH" >> "$TEMP_CRON"
echo "✅ Added: PUT options bot health check every 30 min (09:00-16:00)"

# Add log cleanup (keep only 7 days)
echo "$CRON_CLEANUP" >> "$TEMP_CRON"
echo "✅ Added: Log cleanup (daily, keep 7 days)"

# Add EOD stats export at 4:00 PM (after market close)
echo "$CRON_EXPORT" >> "$TEMP_CRON"
echo "✅ Added: PUT EOD stats export at 16:00 (Mon-Fri)"

echo ""
echo "=========================================="
echo "Preview of new cron jobs:"
echo "=========================================="
grep -E "reboot|router|optbot|options bot|instrument" "$TEMP_CRON" || true

echo ""
echo "=========================================="
echo "Installing crontab..."
echo "=========================================="

# Install new crontab
crontab "$TEMP_CRON"

# Verify installation
echo ""
echo "Installed cron jobs for PUT Options Bot:"
crontab -l | grep -E "reboot|router|optbot|options bot|instrument" || echo "No matching jobs found"

echo ""
echo "=========================================="
echo "✅ PUT Options Bot Cron setup complete!"
echo "=========================================="
echo ""
echo "Schedule:"
echo "  🔄 08:45 AM  - System reboot (5-min delay, done by 08:50)"
echo "  📊 08:50 AM  - Refresh PUT instrument.json from broker"
echo "  🚀 08:55 AM  - Webhook router health check (ensure running)"
echo "  🕘 09:00 AM  - PUT options bot auto-starts (port 8082)"
echo "  🔍 Every 30m - PUT options bot health check (09:00-16:00)"
echo "  📊 16:00 PM  - PUT EOD stats export"
echo "  🗑️  00:00 AM - Cleanup old logs"
echo ""
echo "Backup of old crontab: $CRONTAB_BACKUP"
echo "Restore with: crontab $CRONTAB_BACKUP"
echo ""
echo "Monitor cron logs:"
echo "  tail -f /tmp/put_optbot_instrument_refresh.log"
echo "  tail -f /tmp/router_health.log"
echo "  tail -f /tmp/put_optbot_startup.log"
echo "  tail -f /tmp/put_optbot_health.log"
echo "  tail -f /tmp/put_optbot_eod_export.log"
echo ""

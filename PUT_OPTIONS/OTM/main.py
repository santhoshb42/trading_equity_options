...
# Add PUT_OPTIONS/ for optcode, trading/ and trading/tools/ for shared modules
            ...
            while self.running:
                try:
                    ...
                    if not ltps_refreshed:
                        ...
                        # Still write live data even when skipping exit checks.
...
if __name__ == "__main__":
    ...
    if script_dir.name != "put_options":
        print("Error: This script must be run from the put_options directory")
        sys.exit(1)
    
    # Prevent multiple instances using PID file with atomic locking
    pid_file = script_dir / "put_options_bot.pid"
    lock_file = script_dir / ".put_options_bot.lock"
    ...
    try:
        ...
        try:
            ...
            print(f"❌ ERROR: PUT Options bot already running (another process holds lock)")
        ...
        if pid_file.exists():
            try:
                ...
                if is_our_bot_running(old_pid):
                    print(f"❌ ERROR: PUT Options bot already running (PID {old_pid})")
        ...
        print(f"✅ Started PUT Options bot (PID {os.getpid()})")

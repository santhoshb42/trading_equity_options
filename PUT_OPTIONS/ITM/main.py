                            ...
                            try:
                                ...
                                if self.live_data_formatter:
                                    ...
                                    if csv_data != getattr(self, '_last_live_csv_payload', None):
                                        csv_file = Path(__file__).parent / 'data' / 'live_data_trades.csv'
...
if __name__ == "__main__":
    ...
    # Ensure we're in a recognised bot directory
    script_dir = Path(__file__).parent
    VALID_BOT_DIRS = ("options", "ITM_options", "put_options", "ITM_put_options")
    if script_dir.name not in VALID_BOT_DIRS:
        print(f"Error: This script must be run from one of: {', '.join(VALID_BOT_DIRS)}")
        sys.exit(1)

    # Prevent multiple instances using PID file with atomic locking
    bot_name = script_dir.name
    pid_file = script_dir / f"{bot_name}_bot.pid"
    lock_file = script_dir / f".{bot_name}_bot.lock"

    def is_our_bot_running(pid: int) -> bool:
        """Return True only if PID exists AND its cmdline belongs to main.py in this directory.
        ...
        try:
            ...
            return f'{script_dir.name}/main.py' in cmdline
        except Exception:

...
class LiveDataTableFormatter:
    ...
    @staticmethod
    def _get_overall_summary(live_data: Dict[str, Any]) -> Dict[str, Any]:
        summary = live_data.get('summary')
        if summary is not None:
            return summary

        index_summary = live_data.get('index_summary', {})
        non_index_summary = live_data.get('non_index_summary', {})
        budget_used = float(index_summary.get('budget_used', 0.0)) + float(non_index_summary.get('budget_used', 0.0))
        total_budget = float(live_data.get('total_budget', 0.0) or 0.0)
        if total_budget <= 0:
        ...
        return {
            ...
            'budget_remaining': max(0.0, total_budget - budget_used),
            'budget_used_percent': (budget_used / total_budget * 100) if total_budget > 0 else 0.0,
    ...
    def generate_markdown_summary(self, live_data: Dict[str, Any]) -> str:
        ...
        summary = self._get_overall_summary(live_data)
...
"""
    ...
    def generate_csv(self) -> str:
        ...
        csv_updated_at = datetime.now().isoformat(timespec='seconds')
        json_updated_at = self._get_file_updated_at(self.live_data_file) or 'N/A'
        ...
        # === TIMESTAMP ===
        csv_lines.append(f'live_data_trades.csv Last Updated: {csv_updated_at}')
        csv_lines.append(f'live_data.json Last Updated: {json_updated_at}')
        csv_lines.append("")
        ...
        for trade in today_closed_sorted:
            ...
            # Peak PROFIT reached as % of entry (for tuning TRIAL_SL). ACTION-AWARE: a SHORT profits as
            # premium FALLS, so its profit-peak is the LOWEST premium; a LONG's is the highest. (Fixes
            # shorts showing Peak% < PnL% when premium barely rose but fell into profit.)
            ...
            # Strategy that fired this entry (AO_IGNITION / AO_MOMCONT / RSI_BURN / ...).
            # Already persisted on every record under entry_context; surfaced here so the
...
─────────────────────────────────────────────────────────────────────────
Ongoing Trades      : {summary['ongoing_trades']:>20}

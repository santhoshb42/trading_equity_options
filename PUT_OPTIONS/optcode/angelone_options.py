...
from .optconfig import AngelOneConfig, OptionsTradingConfig, BASE_DIR, DATA_DIR, DevConfig
...
_ORDER_BOOK_CACHE_TTL_SECONDS = 0.35
_ORDER_BOOK_MAX_STALE_SECONDS = 2.0
_ORDER_BOOK_RATE_LIMIT_BASE_COOLDOWN_SECONDS = 1.0
_ORDER_BOOK_RATE_LIMIT_MAX_COOLDOWN_SECONDS = 5.0
_TRADE_BOOK_CACHE_TTL_SECONDS = 1.0
_TRADE_BOOK_MAX_STALE_SECONDS = 5.0
_TRADE_BOOK_RATE_LIMIT_BASE_COOLDOWN_SECONDS = 1.0
_TRADE_BOOK_RATE_LIMIT_MAX_COOLDOWN_SECONDS = 5.0
...
class OptionChain:
    ...
    def get_atm_contracts(self, current_spot: float, offset: int = 0) -> Optional[Tuple[OptionContract, OptionContract]]:
        ...
        # RULE: Pick IMMEDIATE NEXT strike based on direction (offset)
        # offset >= 0: OTM/ITM-PE = first strike AT OR ABOVE alert_price (>= boundary-safe)
        # offset < 0: ITM = immediate next LOWEST strike (last strike < alert_price)
        # For alert_price=406.25 with strikes=[400,405,410,415]
        # OTM (offset>=0): pick 406.25 if on strike, else next higher (s >= alert_price)
        # ITM (offset<0): pick 405 (last strike < 406.25)
        
        if offset >= 0:
            # OTM: next highest strike
            higher_strikes = [s for s in available_strikes if s >= current_spot]
            if higher_strikes:
                selected_strike = higher_strikes[0]  # First strike at or above spot
            else:
                selected_strike = available_strikes[-1]  # If price above all, use highest
            logger.debug(f"ATM: LTP={current_spot} | OTM mode | selected_strike={selected_strike} (next higher) | available={available_strikes[:5]}...")
        else:
            # ITM: next lowest strike
            lower_strikes = [s for s in available_strikes if s < current_spot]
            if lower_strikes:
                selected_strike = lower_strikes[-1]  # Immediate next lower
            else:
                selected_strike = available_strikes[0]  # If price below all, use lowest
            logger.debug(f"ATM: LTP={current_spot} | ITM mode | selected_strike={selected_strike} (next lower) | available={available_strikes[:5]}...")
    ...
    def get_iv_percentile(self) -> Optional[float]:
        ...
        try:
            ...
            # Ignore placeholder chains where every contract carries the same synthetic IV.
            # Those values come from fallback estimation and do not contain usable percentile info.
            unique_iv_values = {round(float(iv), 6) for iv in iv_values}
            if len(unique_iv_values) <= 1:
                logger.debug(
                    f"IV_PERCENTILE: unavailable | flat IV surface detected | "
                    f"iv={next(iter(unique_iv_values)) if unique_iv_values else 'NA'} | contracts={len(iv_values)}"
                )
                return None
...
class AngelOneOptionsBroker:
    ...
    def __init__(self):
        ...
        # Shared order-book snapshot state. This collapses concurrent broker reads
        # into one fetch so BUY/SL polling scales without stampeding SmartAPI.
        self._order_book_cache: Optional[List[Dict[str, Any]]] = None
        self._order_book_cache_time = 0.0
        self._order_book_fetch_lock = threading.Lock()
        self._order_book_rate_limited_until = 0.0
        self._order_book_rate_limit_hits = 0
        self._trade_book_cache: Optional[Dict[str, Any]] = None
        self._trade_book_cache_time = 0.0
        self._trade_book_fetch_lock = threading.Lock()
        self._trade_book_rate_limited_until = 0.0
        self._trade_book_rate_limit_hits = 0
        self._top_gainers_cache: Dict[Tuple[str, str], List[Dict[str, Any]]] = {}
        self._top_gainers_cache_time: Dict[Tuple[str, str], float] = {}
        self._top_gainers_cache_ttl_seconds = 60.0
        self._top_gainers_fetch_lock = threading.Lock()
    ...
    def _get_smartapi_auth_header(self) -> Optional[str]:
        if not self.session_token:
            return None
        if str(self.session_token).startswith("Bearer "):
            return self.session_token
        return f"Bearer {self.session_token}"

    def _build_smartapi_headers(self) -> Dict[str, str]:
        return {
            'Authorization': self._get_smartapi_auth_header() or '',
    ...
    def call_smartapi(self, endpoint: str, method: str = 'POST', payload: Optional[Dict[str, Any]] = None,
                     timeout: float = 5.0, request_type: str = 'oi_fetch') -> Dict[str, Any]:
        """Call a SmartAPI REST endpoint with existing auth and token refresh handling."""
        if not self.ensure_authenticated():
            return {'status': False, 'message': 'Broker authentication unavailable'}

        if not self.smart_api:
            return {'status': False, 'message': 'SmartAPI client unavailable'}

        auth_header = self._get_smartapi_auth_header()
        if not auth_header:
            return {'status': False, 'message': 'Session token unavailable'}

        rate_limiter = get_options_rate_limiter()
        if not rate_limiter.wait_for_call_permission(timeout=5.0, request_type=request_type):
            logger.warning(f"SMARTAPI_CALL: RATE_LIMITED | endpoint={endpoint} | request_type={request_type}")
            return {'status': False, 'message': 'Rate limited'}

        url = endpoint if endpoint.startswith('http') else f"https://apiconnect.angelone.in/rest/secure/angelbroking{endpoint}"
        headers = self._build_smartapi_headers()
        ...
        def _normalize_response(response_json: Dict[str, Any]) -> Dict[str, Any]:
            normalized = response_json if isinstance(response_json, dict) else {}
            normalized['status'] = bool(normalized.get('status') or normalized.get('success'))
            return normalized
        ...
        def _parse_response(response: requests.Response) -> Dict[str, Any]:
            try:
                ...
                message = response.text[:200] if response.text else 'Invalid JSON response'
                return {'status': False, 'message': message}
            return _normalize_response(parsed)

        def _is_invalid_token(response: requests.Response, data: Dict[str, Any]) -> bool:
            message = str(data.get('message', ''))
            error_code = str(data.get('errorcode', ''))
            return response.status_code == 401 or 'Invalid Token' in message or error_code == 'AG8001'
        ...
        try:
            ...
            if _is_invalid_token(response, data):
                logger.warning(f"SMARTAPI_CALL: INVALID_TOKEN | endpoint={endpoint} | reauth_retry=1")
                rate_limiter.record_call(request_type, False)
                if not self._handle_invalid_token_error():
                    return data
            ...
            success = 200 <= response.status_code < 300 and data.get('status')
            rate_limiter.record_call(request_type, success)

            if not success:
                if response.status_code < 200 or response.status_code >= 300:
                    data.setdefault('message', response.text[:200] if response.text else f'HTTP {response.status_code}')
                logger.warning(
                    f"SMARTAPI_CALL: FAILED | endpoint={endpoint} | status_code={response.status_code} | message={data.get('message', 'Unknown error')}"
                )
            return data
        except Exception as exc:
            rate_limiter.record_call(request_type, False)
            logger.warning(f"SMARTAPI_CALL: ERROR | endpoint={endpoint} | {type(exc).__name__}: {str(exc)}")
            return {'status': False, 'message': str(exc)}

    def _set_last_order_error(self, error_message: str, error_code: str = "") -> None:
        self.last_order_error = str(error_message or "")
        self.last_order_error_code = str(error_code or "")

    def _clear_last_order_error(self) -> None:
        self.last_order_error = ""
        self.last_order_error_code = ""

    def _is_no_funds_error(self, error_message: str = "", error_code: str = "") -> bool:
        haystack = f"{error_code} {error_message}".lower()
        keywords = (
            'insufficient funds',
            'no funds',
    ...
    def get_funds_snapshot(self, force_refresh: bool = False) -> Optional[Dict[str, Any]]:
        """Fetch current Angel One RMS/funds snapshot for entry gating."""
        del force_refresh

        if not self.ensure_authenticated():
            logger.error("BROKER_FUNDS: AUTH_FAILED | unable to verify funds")
            return None

        if not self.smart_api or not hasattr(self.smart_api, 'rmsLimit'):
            logger.error("BROKER_FUNDS: RMS_UNAVAILABLE | SmartAPI rmsLimit missing")
            return None

        response = call_with_timeout(self.smart_api.rmsLimit, 5.0)
        if not response:
            logger.error("BROKER_FUNDS: EMPTY_RESPONSE | rmsLimit returned no data")
            return None

        if isinstance(response, dict):
            if response.get('status') is False:
                message = response.get('message', 'Unknown RMS error')
                logger.error(f"BROKER_FUNDS: RMS_FAILED | message={message}")
                if 'AG8001' in str(message):
                    self._handle_invalid_token_error()
                return None
            data = response.get('data') if isinstance(response.get('data'), dict) else response
        else:
            logger.error(f"BROKER_FUNDS: INVALID_RESPONSE | type={type(response).__name__}")
            return None

        available_cash = self._safe_float(
            data.get('availablecash'),
            data.get('availableCash'),
            data.get('availableintradaypayin'),
            data.get('availablelimitmargin'),
            data.get('net'),
        )
        net_amount = self._safe_float(data.get('net'))
        ...
        logger.info(
            f"BROKER_FUNDS: SNAPSHOT | available_cash=₹{available_cash:.2f} | net=₹{net_amount:.2f}"
        )
        return snapshot
    ...
    def fetch_option_chain(self, underlying: str, expiry: str, current_price: Optional[float] = None, force_refresh: bool = False, light: bool = False) -> Optional[OptionChain]:
        ...
        try:
            normalized_price = round(float(current_price), 2) if current_price else None
            key = (underlying, expiry, normalized_price)

            # Keep option-chain caching behavior identical across PAPER and LIVE.
            # current_price influences ATM centering, so it must be part of the cache key.
            should_use_cache = not force_refresh

            # Check cache (valid for 10 seconds - matches monitoring cycle)
            ...
            if not rate_limiter.wait_for_call_permission(timeout=30.0, request_type="place_order"):
                ...
                def chain_callback():
                    return self.fetch_option_chain(underlying, expiry, current_price=current_price)
    ...
    def _fetch_from_angel(self, underlying: str, expiry: str, current_price: Optional[float] = None, light: bool = False) -> Optional[OptionChain]:
        ...
        # 2nd strike (7 "no strike (next) listed" rejects on 08-05). Count-based works for any
        # strike spacing and stays perf-safe on tight-interval names (±K, not ±pts).
    ...
    def get_next_expiry(self, underlying: str) -> str:
        ...
        extractor = getattr(self, 'pe_extractor', None)
    ...
    def place_options_order(self,
        ...
        if not self.authenticated and OptionsTradingConfig.TRADING_MODE != "PAPER":
            ...
            self._set_last_order_error("Not authenticated")
        ...
        try:
            ...
            try:
                ...
                instrument_token = self.get_instrument_token(symbol, derivatives_exchange)
                if not instrument_token:
                    logger.error(f"ORDER_PLACE: No token found | {symbol} | Cannot place LIVE order")
                    self._set_last_order_error("Instrument token not found")
                    return None
                order_params = {
                ...
                if order_type == "LIMIT" and price > 0:
                    ...
                    # STOPLOSS-MARKET: ONLY set triggerprice, leave price="0"
                    # Setting price alongside triggerprice converts it to STOPLOSS_LIMIT,
                    # which won't fill if market gaps past the limit on a fast move.
                ...
                if isinstance(response, str):
                    ...
                    error_code = "NO_FUNDS" if self._is_no_funds_error(error_msg) else ""
                    if error_code == "NO_FUNDS":
                        logger.warning(f"ORDER_PLACE: NO_FUNDS | {symbol} | response={error_msg}")
                    else:
                        logger.error(f"ORDER_PLACE: LIVE FAILED | {symbol} | response={error_msg}")
                    self._set_last_order_error(error_msg, error_code)
                    print(f"❌ [LIVE] Order placement failed: {error_msg}")
                    return None
            except Exception as live_err:
                live_error_message = str(live_err)
                error_code = "NO_FUNDS" if self._is_no_funds_error(live_error_message) else ""
                if error_code == "NO_FUNDS":
                    logger.warning(f"ORDER_PLACE: NO_FUNDS | {symbol} | {live_error_message}")
                else:
                    logger.error(f"ORDER_PLACE: LIVE ERROR | {symbol} | {live_error_message}")
                self._set_last_order_error(live_error_message, error_code)
                print(f"❌ [LIVE] Error placing order: {live_error_message}")
                return None
        
        except Exception as e:
            logger.error(f"ORDER_PLACE: ERROR | {symbol} | {str(e)}", symbol=symbol, action=action)
            rate_limiter.record_call("place_order", False)
            self._set_last_order_error(str(e))
    ...
    def wait_for_buy_confirmation(self, symbol: str, timeout: int = 30, order_id: Optional[str] = None) -> bool:
        ...
        while time.time() - start_time < timeout:
            try:
                order_status = self.get_order_status(order_id, max_cache_age=poll_interval)
                if not order_status:
                    time.sleep(poll_interval)
                    continue

                broker_status = str(order_status.get('status', '')).upper()

                if broker_status in ['COMPLETE', 'FILLED', 'FULLY_FILLED']:
                    logger.info(f"BUY_CONFIRM: ORDER_FILLED | {symbol} | order_id={order_id} | status={broker_status} | elapsed={time.time() - start_time:.1f}s")
                ...
                if broker_status in ['REJECTED', 'CANCELLED', 'EXPIRED']:
                    logger.error(f"BUY_CONFIRM: ORDER_{broker_status} | {symbol} | order_id={order_id}")
                    self.pending_buy_orders.pop(order_id, None)
                    if self.pending_buy_orders_by_symbol.get(tracked_symbol) == order_id:
                        self.pending_buy_orders_by_symbol.pop(tracked_symbol, None)
                    return False

                logger.debug(f"BUY_CONFIRM: WAITING | {symbol} | status={broker_status} | elapsed={time.time() - start_time:.1f}s")
                time.sleep(poll_interval)
        ...
        final_status = self.get_order_status(order_id, force_refresh=True, max_cache_age=0.0)
        if final_status:
            broker_status = str(final_status.get('status', '')).upper()
            if broker_status in ['COMPLETE', 'FILLED', 'FULLY_FILLED']:
                logger.warning(f"BUY_CONFIRM: LATE_FILL_DETECTED | {symbol} | order_id={order_id} | status={broker_status}")
                pending_order['status'] = 'FILLED'
                self.pending_buy_orders.pop(order_id, None)
                if self.pending_buy_orders_by_symbol.get(tracked_symbol) == order_id:
                    self.pending_buy_orders_by_symbol.pop(tracked_symbol, None)
    ...
    def get_instrument_token(self, symbol: str, exchange: str = "NFO") -> Optional[str]:
        ...
        try:
            self.pe_extractor.reload_if_stale()

            # Use CE extractor to lookup token
            instruments = self.pe_extractor.instruments
            ...
            if self.pe_extractor.reload_if_stale():
                instruments = self.pe_extractor.instruments
                if symbol in instruments:
                    logger.info(f"TOKEN_LOOKUP: RECOVERED_AFTER_RELOAD | {symbol} | exchange={exchange}")
                    return instruments[symbol].get('token', '')
    ...
    def _normalize_top_gainer_symbol(self, value: Any) -> str:
        """Normalize SmartAPI top-gainers symbols to underlying names used by filters."""
        symbol = str(value or '').strip().upper()
        if not symbol:
            return ''

        symbol = symbol.replace('-EQ', '').replace(' EQ', '').replace('_EQ', '')
        symbol = re.sub(r'\d{1,2}[A-Z]{3}\d{2}', '', symbol)
        symbol = symbol.replace('FUT', '').strip()
        return symbol

    def get_top_gainers(self, limit: int = 15, datatype: Optional[str] = None,
                        expiry_type: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Fetch top gainers from SmartAPI and normalize them for entry filters.

        SmartAPI's published sample uses:
        {"datatype": "PercOIGainers", "expirytype": "NEAR"}

        Returns:
            List of dicts with at least: symbol, rank, raw_symbol
        """
        requested_limit = max(int(limit or 0), 1)
        payload_datatype = str(datatype or os.getenv('CALL_OPTIONS_TOP_GAINERS_DATATYPE', 'PercOIGainers')).strip() or 'PercOIGainers'
        payload_expiry_type = str(expiry_type or os.getenv('CALL_OPTIONS_TOP_GAINERS_EXPIRY_TYPE', 'NEAR')).strip().upper() or 'NEAR'
        cache_key = (payload_datatype, payload_expiry_type)
        now = time.time()

        cached = self._top_gainers_cache.get(cache_key)
        cached_at = self._top_gainers_cache_time.get(cache_key, 0.0)
        if cached and (now - cached_at) < self._top_gainers_cache_ttl_seconds:
            return cached[:requested_limit]

        with self._top_gainers_fetch_lock:
            cached = self._top_gainers_cache.get(cache_key)
            cached_at = self._top_gainers_cache_time.get(cache_key, 0.0)
            now = time.time()
            if cached and (now - cached_at) < self._top_gainers_cache_ttl_seconds:
            ...
            if not self.ensure_authenticated() or not self.smart_api:
                logger.debug("TOP_GAINERS: Broker unavailable")
                return []

            payload = {
                'datatype': payload_datatype,
                'expirytype': payload_expiry_type,
            }

            try:
                if hasattr(self.smart_api, 'gainersLosers'):
                    response = call_with_timeout(self.smart_api.gainersLosers, 5.0, payload)
                else:
                    response = self.call_smartapi(
                        endpoint='/marketData/v1/gainersLosers',
                        method='POST',
                        payload=payload,
                        timeout=5.0,
                        request_type='oi_fetch'
                    )
            except Exception as exc:
                logger.debug(f"TOP_GAINERS: API_ERROR | {type(exc).__name__}: {str(exc)}")
                return []

            if not isinstance(response, dict) or not response.get('status'):
                logger.debug(f"TOP_GAINERS: API_FAILED | payload={payload} | response_type={type(response).__name__}")
                return []

            data = response.get('data', [])
            if not isinstance(data, list) or not data:
                logger.debug(f"TOP_GAINERS: EMPTY | payload={payload}")
                return []
            ...
            for index, item in enumerate(data, start=1):
                ...
                symbol = self._normalize_top_gainer_symbol(raw_symbol)
                if not symbol:
                    continue

                change_pct = None
                for key in (
                    'percentChange', 'percentageChange', 'pChange', 'changePercent',
                    'chgPerc', 'netChangePercent', 'percentchange', 'change_percentage'
                ):
                    value = item.get(key)
                    if value in (None, ''):
                        continue
            ...
            if not normalized:
                logger.debug(f"TOP_GAINERS: PARSE_FAILED | payload={payload} | rows={len(data)}")
                return []

            self._top_gainers_cache[cache_key] = normalized
            self._top_gainers_cache_time[cache_key] = now
            logger.debug(
                f"TOP_GAINERS: SUCCESS | datatype={payload_datatype} | expiry={payload_expiry_type} | count={len(normalized)}"
            )
    ...
    def get_historical_data(self, symbol: str, interval: str = "FIVE_MINUTE",
        ...
        """
        try:
            use_mock_fallback = DevConfig.PAPER_TRADING_ENABLED
            if not self.authenticated:
    ...
    def _get_cached_order_book(self, max_age: Optional[float] = None, allow_stale: bool = False) -> Optional[List[Dict[str, Any]]]:
        if self._order_book_cache is None or self._order_book_cache_time <= 0:
            return None

        age = time.time() - self._order_book_cache_time
        ttl = _ORDER_BOOK_MAX_STALE_SECONDS if allow_stale else (max_age if max_age is not None else _ORDER_BOOK_CACHE_TTL_SECONDS)
        if age <= ttl:
            return list(self._order_book_cache)
        return None

    def _set_order_book_cache(self, orders: List[Dict[str, Any]]) -> None:
        self._order_book_cache = list(orders or [])
        self._order_book_cache_time = time.time()

    def _is_rate_limit_response(self, payload: Any) -> bool:
    ...
    def _note_order_book_rate_limit(self, reason: Any) -> None:
        self._order_book_rate_limit_hits += 1
        cooldown = min(
            _ORDER_BOOK_RATE_LIMIT_BASE_COOLDOWN_SECONDS * (2 ** (self._order_book_rate_limit_hits - 1)),
            _ORDER_BOOK_RATE_LIMIT_MAX_COOLDOWN_SECONDS,
        )
        self._order_book_rate_limited_until = time.time() + cooldown
        logger.warning(
            f"ORDER_BOOK: RATE_LIMIT_COOLDOWN | cooldown={cooldown:.2f}s | hits={self._order_book_rate_limit_hits} | reason={reason}"
        )

    def _clear_order_book_rate_limit(self) -> None:
        self._order_book_rate_limit_hits = 0
        self._order_book_rate_limited_until = 0.0

    def get_order_book_snapshot(self, force_refresh: bool = False, max_cache_age: Optional[float] = None) -> Optional[List[Dict[str, Any]]]:
        try:
            if not self.authenticated:
            ...
            cached_orders = None if force_refresh else self._get_cached_order_book(max_age=max_cache_age)
            if cached_orders is not None:
                return cached_orders

            now = time.time()
            if not force_refresh and now < self._order_book_rate_limited_until:
                stale_orders = self._get_cached_order_book(allow_stale=True)
                if stale_orders is not None:
                    logger.debug("ORDER_BOOK: USING_STALE_CACHE during broker cooldown")
                    return stale_orders
                logger.warning("ORDER_BOOK: SKIPPED during broker cooldown with no cached snapshot")
                return None

            acquired = self._order_book_fetch_lock.acquire(timeout=0.25)
            if not acquired:
                coalesced_orders = self._get_cached_order_book(max_age=max_cache_age, allow_stale=True)
                if coalesced_orders is not None:
                    logger.debug("ORDER_BOOK: COALESCED_CACHE_HIT")
                    return coalesced_orders
                logger.warning("ORDER_BOOK: FETCH_BUSY with no cached snapshot")
                return None
            ...
            try:
                ...
                # Wait for rate limit permission
                if not rate_limiter.wait_for_call_permission(timeout=5.0, request_type="get_order_book"):
                    logger.warning("ORDER_BOOK: RATE_LIMITED")
                    stale_orders = self._get_cached_order_book(allow_stale=True)
                    return stale_orders if stale_orders is not None else None
                ...
                if response and response.get('status'):
                    ...
                    self._set_order_book_cache(orders)
                    self._clear_order_book_rate_limit()
                ...
                if self._is_rate_limit_response(response):
                    self._note_order_book_rate_limit(response)
                rate_limiter.record_call("get_order_book", False)
                stale_orders = self._get_cached_order_book(allow_stale=True)
                return stale_orders if stale_orders is not None else None
            finally:
                self._order_book_fetch_lock.release()
        
        except Exception as e:
            logger.error(f"ORDER_BOOK: ERROR | {str(e)}")
            if self._is_rate_limit_response(e):
                self._note_order_book_rate_limit(e)
            rate_limiter = get_options_rate_limiter()
            rate_limiter.record_call("get_order_book", False)
            stale_orders = self._get_cached_order_book(allow_stale=True)
            return stale_orders if stale_orders is not None else None

    def get_order_status(self, order_id: str, force_refresh: bool = False, max_cache_age: Optional[float] = None) -> Optional[Dict[str, Any]]:
        """Lookup a single AngelOne order by id and normalize its status fields."""
        try:
            order_book = self.get_order_book_snapshot(force_refresh=force_refresh, max_cache_age=max_cache_age)
            if not order_book:
                return None
    ...
    def _get_cached_trade_book(self, max_age: Optional[float] = None, allow_stale: bool = False) -> Optional[Dict[str, Any]]:
        if self._trade_book_cache is None or self._trade_book_cache_time <= 0:
            return None

        age = time.time() - self._trade_book_cache_time
        ttl = _TRADE_BOOK_MAX_STALE_SECONDS if allow_stale else (max_age if max_age is not None else _TRADE_BOOK_CACHE_TTL_SECONDS)
        if age <= ttl:
            return dict(self._trade_book_cache)
        return None

    def _set_trade_book_cache(self, trade_book: Dict[str, Any]) -> None:
        self._trade_book_cache = dict(trade_book or {})
        self._trade_book_cache_time = time.time()

    def _note_trade_book_rate_limit(self, reason: Any) -> None:
        self._trade_book_rate_limit_hits += 1
        cooldown = min(
            _TRADE_BOOK_RATE_LIMIT_BASE_COOLDOWN_SECONDS * (2 ** (self._trade_book_rate_limit_hits - 1)),
            _TRADE_BOOK_RATE_LIMIT_MAX_COOLDOWN_SECONDS,
        )
        self._trade_book_rate_limited_until = time.time() + cooldown
        logger.warning(
            f"TRADE_BOOK: RATE_LIMIT_COOLDOWN | cooldown={cooldown:.2f}s | hits={self._trade_book_rate_limit_hits} | reason={reason}"
        )

    def _clear_trade_book_rate_limit(self) -> None:
        self._trade_book_rate_limit_hits = 0
        self._trade_book_rate_limited_until = 0.0
    ...
    def get_trade_book(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            cached_trade_book = self._get_cached_trade_book()
            if cached_trade_book is not None:
                return cached_trade_book

            now = time.time()
            if now < self._trade_book_rate_limited_until:
                stale_trade_book = self._get_cached_trade_book(allow_stale=True)
                if stale_trade_book is not None:
                    logger.debug("TRADE_BOOK: USING_STALE_CACHE during broker cooldown")
                    return stale_trade_book
                logger.warning("TRADE_BOOK: SKIPPED during broker cooldown with no cached snapshot")
                return None

            acquired = self._trade_book_fetch_lock.acquire(timeout=0.25)
            if not acquired:
                coalesced_trade_book = self._get_cached_trade_book(allow_stale=True)
                if coalesced_trade_book is not None:
                    logger.debug("TRADE_BOOK: COALESCED_CACHE_HIT")
                    return coalesced_trade_book
                logger.warning("TRADE_BOOK: FETCH_BUSY with no cached snapshot")
                return None
            ...
            try:
                ...
                # Wait for rate limit permission
                if not rate_limiter.wait_for_call_permission(timeout=5.0, request_type="get_trade_book"):
                    logger.warning("TRADE_BOOK: RATE_LIMITED")
                    stale_trade_book = self._get_cached_trade_book(allow_stale=True)
                    return stale_trade_book if stale_trade_book is not None else None
                ...
                if response and response.get('status'):
                    ...
                    for trade in trades:
                        ...
                        action = trade.get('ordertype', '').upper()

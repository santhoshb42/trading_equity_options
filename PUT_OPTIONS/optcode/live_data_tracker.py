...
class LiveDataTracker:
    ...
    def poll_post_exit_tracking(self) -> None:
        ...
        # Pre-fetch LTPs for all watchers DUE this poll in ONE bulk call (chunked at 50),
        # instead of a per-symbol get_market_data inside the loop. Matters when many positions
        # close together (EOD / stale-exit waves) — keeps post-exit telemetry from multiplying
        # quote-endpoint calls. Non-due watchers are skipped by the same throttle check below.
        _due = []
    ...
    def close_trade(self,
        ...
        for trade in self.live_data['trades']:
            if trade['symbol'] == symbol and trade['status'] == 'OPEN':
                ...
                # Peak reached during the trade (for tuning the TRIAL_SL). peak_pct = max premium
                # excursion vs entry — how far this trade actually ran. Mirrors CE.

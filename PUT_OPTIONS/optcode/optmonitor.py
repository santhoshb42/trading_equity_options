...
class OptionPositionMonitor:
    ...
    def add_position(self,
        ...
        try:
            ...
            _sl_pct0 = OptionsTradingConfig.STOP_LOSS_PERCENTAGE
            if getattr(position, 'action', 'BUY') == 'SELL':
                position.hard_sl_price = self._round_to_10_paise(
                    position.entry_premium * (1 + _sl_pct0 / 100)
                )
            else:
                position.hard_sl_price = self._round_to_10_paise(
                    position.entry_premium * (1 - _sl_pct0 / 100)
            ...
            # Log trade entry to CSV (order already placed above, so latency does not matter)
            try:
                ...
                logger.info(
                    ...
                    f"EMA9up={tech_snapshot.get('calc_ema9_up3','?')} | "
                    f"EMA20up={tech_snapshot.get('calc_ema20_up3','?')} | "
    ...
    def place_stop_loss_order(self, symbol: str) -> bool:
        ...
        _sl_pct = OptionsTradingConfig.STOP_LOSS_PERCENTAGE
        _is_short = getattr(position, 'action', 'BUY') == 'SELL'
        if _is_short:
    ...
    def retry_failed_sl_orders(self) -> Dict[str, int]:
        ...
        for symbol, position in self._snapshot_positions_items():
            ...
            _slp = OptionsTradingConfig.STOP_LOSS_PERCENTAGE
    ...
    def check_stale_consolidation_exits(self) -> List[Dict[str, Any]]:
        ...
        # Threshold from data analysis. 2026-08-18: made env-driven to MIRROR CE. This was
        # HARDCODED to 10 here while CE had been env-driven since 2026-07-24, so every
        # OPTIONS_STALE_CONSOLIDATION_MINUTES change silently applied to the CE bots only and the
        # PE bots kept cutting at 10min regardless of what the .env said.

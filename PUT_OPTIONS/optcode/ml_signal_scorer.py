...
from __future__ import annotations  # lazy annotation eval — np.ndarray in return hint doesn't require numpy at load time
import json
import pickle
...
# sklearn imports deferred to MLSignalScorer.__init__ — avoids loading scipy/openblas
# (~150MB) at bot startup when no trained model files exist.
HAS_SKLEARN = False  # updated at first instantiation if sklearn available + models present
...
class OptionsFeatureExtractor:
    ...
    def extract_features(alert: Dict[str, Any], 
        ...
        if avg_volume > 0:
            ...
            volume_zscore = 0.0
        features['volume_zscore'] = max(-3.0, min(3.0, volume_zscore))
        ...
        features['recent_volatility'] = max(0.0, min(5.0, features['recent_volatility']))
        ...
        features['premium_momentum'] = max(-1.0, min(1.0, features['premium_momentum']))
    ...
    def normalize_features(features: Dict[str, float], 
        ...
        """
        import numpy as np  # only loaded when ML models are active
        # Extract in order
...
class MLSignalScorer:
    ...
    def __init__(self):
        ...
        # Check if any model files actually exist before loading sklearn (~150MB)
        model_files = [
            MLScorerConfig.RANDOM_FOREST_MODEL,
            MLScorerConfig.GRADIENT_BOOSTING_MODEL,
            MLScorerConfig.SCALER_MODEL,
        ]
        has_model_files = any(Path(p).exists() for p in model_files if p)

        global HAS_SKLEARN
        if has_model_files and not HAS_SKLEARN:
            try:
        ...
        self.random_forest = self._load_model(MLScorerConfig.RANDOM_FOREST_MODEL) if has_model_files else None
        self.gradient_boosting = self._load_model(MLScorerConfig.GRADIENT_BOOSTING_MODEL) if has_model_files else None
        self.scaler = self._load_model(MLScorerConfig.SCALER_MODEL) if has_model_files else None
        ...
        if self.models_available:
            ...
            logger.debug("ML_SCORER: NO_MODELS | using fallback scoring (sklearn not loaded)")
    ...
    def score_signal(self, alert: Dict[str, Any],
        ...
        try:
            ...
            # Clamp to safe range
            win_prob = max(MLScorerConfig.ML_SCORE_MIN, min(MLScorerConfig.ML_SCORE_MAX, win_prob))
            ...
            confidence_adjusted = max(0.0, min(100.0, confidence_adjusted))
    ...
    def _ensemble_predict(self, features: Dict[str, float]) -> float:
        ...
        # Weighted average
        if predictions:
            w = weights[:len(predictions)]
            ensemble_prob = sum(p * wt for p, wt in zip(predictions, w)) / sum(w)

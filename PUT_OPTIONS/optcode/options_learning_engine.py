...
class LearningConfig:
    ...
    SYMBOLS = ['BANKNIFTY', 'NIFTY', 'FINNIFTY', 'MIDCPNIFTY', 'NIFTYNXT50']
    ...
    # Data persistence - paths resolved via DATA_DIR so OTM and ITM stay isolated
    SYMBOL_STATS_FILE = str(_DATA_DIR / "learning" / "symbol_stats.json")
    TRADE_HISTORY_FILE = str(_DATA_DIR / "trade_history.jsonl")
    MODEL_PERFORMANCE_FILE = str(_DATA_DIR / "model_performance.json")
...
class SymbolPerformanceTracker:
    ...
    def _update_metrics(self, symbol: str) -> None:
        ...
        if stats['total_trades'] >= LearningConfig.MIN_TRADES_FOR_STATS:
            ...
            if profits:
                _mean = sum(profits) / len(profits)
                profit_variance = sum((x - _mean) ** 2 for x in profits) / len(profits)
            else:
                profit_variance = 1000

...
from .optconfig import BASE_DIR, DATA_DIR, OptionsTradingConfig
...
class InstrumentCEExtractor:
    ...
    def build_real_option_chain(self, underlying: str, expiry: str, center_price: Optional[float] = None) -> Optional[List[Dict]]:
        ...
        def symbol_matches_requested_expiry(symbol: str) -> bool:
            upper_symbol = str(symbol or '').strip().upper()
            if not upper_symbol.endswith(('CE', 'PE')):
                return False

            day_mon_yy = expiry_date.strftime('%d%b%y').upper()
            day_mon = expiry_date.strftime('%d%b').upper()

            if re.match(rf'^[A-Z0-9]+?{day_mon_yy}\d+(CE|PE)$', upper_symbol):
                return True

            if re.match(rf'^[A-Z0-9]+?{day_mon}\d+(CE|PE)$', upper_symbol):
                return True

            return False

        symbol_aligned_contracts = [contract for contract in contracts if symbol_matches_requested_expiry(contract['symbol'])]
        if symbol_aligned_contracts:
            dropped_count = len(contracts) - len(symbol_aligned_contracts)
            if dropped_count > 0:
                logger.info(
                    f"Filtered mismatched expiry symbols | underlying={underlying_clean} | requested={expiry_pattern} | "
                    f"kept={len(symbol_aligned_contracts)} | dropped={dropped_count}"
                )
            contracts = symbol_aligned_contracts
    ...
    def export_ce_pe_cache(self, output_file: Optional[Path] = None) -> bool:
        ...
        try:
            output_file = output_file or (DATA_DIR / "ce_pe_map.json")

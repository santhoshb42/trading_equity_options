...
class OptionPositionMonitor:
    ...
    def _progressive_trailing_gap(self, peak_gain_percent: float) -> float:
        ...
        A constant tight gap clips the rare big runners that carry the P&L; widening the gap as the
        trade proves itself locks a rising FRACTION of the peak (give-back shrinks in %, absolute
        room grows), so runners are given room to breathe. Tiers env-tunable via OPTIONS_TRIAL_GAP_*.
            peak +5..+12% -> 4%   |   +25..+60% -> 9%
            peak +12..+25% -> 6%  |   +60%+     -> 12%
        """
    ...
    def add_position(self,
        ...
        try:
            ...
            if _idx_profile:
                # H16c: index trail arms much higher — noise at index leverage reaches the stock
                # arm level constantly; 12% arm was the consistent winner across the index grid.
                expected_threshold = _idx_profile['trial_activation']
                market_reason = f"INDEX_PROFILE ({position.underlying})"
    ...
    def _close_position_inner(
        ...
        *,
        ...
        try:
            ...
            # 2026-07-24: refresh the underlying's spot ONCE at exit so underlying_exit_price /
            # underlying_move_pct in the trade record are real. The per-cycle update lived in
            # refresh_position_greeks() which is never wired into the loop, so current_underlying_
            # price stayed frozen at the alert price -> every record showed 0.0% move (made the
            # UNITDSPR-type real gains look phantom). Post-SELL + best-effort = never delays exit.
            try:
                ...
                pass  # telemetry only — never block the exit
    ...
    def _nifty_md_cached(self, ttl: float = 5.0):
        ...
        (~1,457 calls/bot/day — the single largest rate-limit source), yet NIFTY is identical for
        every position in a cycle. One shared fetch per TTL window instead."""
        import time as _t
        ...
        try:
            ...
            md = c[1] if c else None   # reuse last good value on a transient failure
    ...
    def check_trailing_stop_losses(self) -> List[Dict[str, Any]]:
        ...
        for symbol, position in positions:
            ...
            if is_trial_sl_enabled:
                ...
                #
                # The gap now WIDENS as the trade proves itself (see _progressive_trailing_gap),
                # replacing the old fixed 2% gap. Regime-change trades are fat-tailed — the money is
                # in the rare +30-280% runners, and a constant tight gap clipped them. Widening the
                # gap with peak locks a rising FRACTION of the peak (give-back shrinks in %, absolute
                # room grows), so runners breathe while modest winners still lock near the arm floor.
            ...
            if position.current_premium <= effective_sl:
                ...
                # PHANTOM-TICK GUARD (2026-07-24): the HARD_SL needs the breach to PERSIST so a lone
                # stale/bad LTP can't fire an instant stop. TRIAL_SL / BREAKEVEN fire immediately.
    ...
    def check_hard_stop_loss(self) -> List[Dict[str, Any]]:
        ...
        for symbol, position in positions:
            ...
            # H13.2 EARLY-LOSER CUT: tighter stop during the position's first N minutes.
            # Runs BEFORE the -10% check so a fast-failing entry exits at -6% instead of
            # riding to a full HARD_SL. Never fires once TRIAL_SL is armed (winner-shaped).
            _ip = OptionsTradingConfig.index_exit_profile(position.underlying)
            _ec_enabled = _ip['early_cut_enabled'] if _ip else OptionsTradingConfig.EARLY_LOSER_CUT_ENABLED
            _ec_pct = _ip['early_cut_pct'] if _ip else OptionsTradingConfig.EARLY_LOSER_CUT_PCT
            if (_ec_enabled
                    and not position.trial_sl_enabled
                    and isinstance(position.entry_time, datetime)
                    and (datetime.now() - position.entry_time).total_seconds()
                        <= OptionsTradingConfig.EARLY_LOSER_CUT_MINUTES * 60):
                early_cut_price = position.entry_premium * (1 - _ec_pct / 100)
                if position.current_premium <= early_cut_price:
                    ...
                    logger.warning(f"EARLY_LOSER_CUT: {symbol} | Entry: ₹{position.entry_premium:.2f} | "
                                   f"Current: ₹{position.current_premium:.2f} | Loss: {loss_percent:.1f}% | "
                                   f"age={age_min:.1f}min <= {OptionsTradingConfig.EARLY_LOSER_CUT_MINUTES:.0f}min | "
                                   f"cut@-{_ec_pct:.0f}%")
                    pnl = self._reconcile_broker_stop_exit(
                        symbol,
                        position.current_premium,
                        f"EARLY_LOSER_CUT (-{_ec_pct:.0f}% within "
                        f"{OptionsTradingConfig.EARLY_LOSER_CUT_MINUTES:.0f}min of entry)"
                    )
                    if pnl:
                        closed.append(pnl)
                        logger.warning(f"EARLY_LOSER_CUT_EXECUTED: {symbol} | PnL: ₹{pnl.get('pnl', 0):.2f}")
                    continue
            ...
            if position.current_premium <= position.hard_sl_price:
                ...
                # PHANTOM-TICK GUARD: a true HARD_SL must persist; breakeven give-backs fire now.
    ...
    def _h19_underlying_rsi(self, underlying: str, period: int, need: int) -> List[Optional[float]]:
        ...
        if hit and hit[0] == now_min:
            return hit[1]
        # force_refresh: the shared candle cache has a 300s TTL — without bypassing it H19
        # evaluates "RSI falling 3 bars" on up-to-5-min-stale data and effectively never fires
        # (AUDIT 2026-07-23: 0 fires in 3 days). Bounded cost: the per-minute cache above caps
        # this at ~1 forced fetch per underlying per minute, un-armed positions only.
    ...
    def check_h19_rsi_exit(self) -> List[Dict[str, Any]]:
        if OptionsTradingConfig.STRATEGY_MODE == 'SELL_THETA':
            return []  # naked theta-sell: no intermediate exits, EOD square-off only
        """H19: exit when the underlying's 1-min RSI falls FALL_BARS consecutive closed bars.
        Replaces EARLY_LOSER_CUT + PROFIT_FLOOR. TRIAL_SL/HARD_SL/STALE remain. PAPER study flag."""
        from optcode.optconfig import SentimentConfig
        ...
        fall = SentimentConfig.H19_RSI_FALL_BARS
        positions = self._snapshot_positions_items()
        for symbol, position in positions:
            ...
            if not underlying:
                continue
            rsi = self._h19_underlying_rsi(underlying, period, fall)
            ...
            if all(window[i] < window[i - 1] for i in range(1, len(window))):
                pnl = self.close_position(
                    ...
                    f"H19_RSI_EXIT (RSI fell {fall} bars: {window[0]:.1f}->{window[-1]:.1f})"
                )
                if pnl:
                    closed.append(pnl)
                    logger.warning(
    ...
    def check_stale_consolidation_exits(self) -> List[Dict[str, Any]]:
        ...
        # Disabled → rely purely on TRIAL_SL / HARD_SL (no time-based stale cut).
        from optcode.optconfig import SentimentConfig
        ...
        # Threshold from data analysis. 2026-07-24: made env-driven and cut 10 -> 5 min.
        # STALE (premium-peak based) is the ONLY exit that catches our dominant loser type:
        # options bleeding to -16% via theta/IV/spread decay while the UNDERLYING is flat
        # (+/-0.3%). H19 watches the underlying's RSI so it's blind to these (it fired 2/8);
        # HARD_SL 18% is too wide (they stop ~-16% and sit). Cutting at 5 min instead of 10
        # would have saved ~Rs7.5K on 07-24's 9 STALE trades (most were less-negative at 5min).
        stale_hold_time_min = float(os.getenv("OPTIONS_STALE_CONSOLIDATION_MINUTES", "10"))
    ...
    def check_stale_positions(self) -> List[Dict[str, Any]]:
        ...
        # Disabled → rely purely on TRIAL_SL / HARD_SL (no time-based stale cut). Mirrors the
        # STALE_CONSOLIDATION toggle; both off = trailing-SL-only exit regime.
        from optcode.optconfig import SentimentConfig
    ...
    def perform_periodic_monitoring(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # H19 (2026-07-21): RSI-falling-3-bars exit on the underlying — replaces EARLY_LOSER_CUT
            # + PROFIT_FLOOR. Runs after trail (winners protected by TRIAL_SL, skipped inside).
            h19_closes = [] if _sell_mode else [
                p for p in self.check_h19_rsi_exit()
                if p['symbol'] not in trailing_closed_symbols

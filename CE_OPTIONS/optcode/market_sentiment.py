...
class MarketSentiment:
    ...
    def fetch_oi_buildup(self, buildup_type: str = 'Long Built Up',
        ...
        # Fast path — return cached result if fresh (no lock needed for reads)
            ...
            if hasattr(self.broker, 'call_smartapi'):
                ...
                if not response or not response.get('status'):
                    ...
                    for item in response.get('data', []):
                        ...
                        base_symbol = re.sub(r'\d{1,2}[A-Z]{3}\d{2}', '', trading_symbol)
                        base_symbol = base_symbol.replace('FUT', '').strip().upper()
    ...
    def get_symbol_sentiment(self, symbol: str, include_exit_data: bool = True) -> Dict[str, Any]:
        """Get sentiment data for symbol, optionally including exit-oriented OI checks."""
        try:
            # Fetch PCR
            pcr_map = self.fetch_pcr_ratio()
            pcr = pcr_map.get(symbol)
            ...
            short_buildup = self.fetch_oi_buildup('Short Built Up').get(symbol) if include_exit_data else None
            short_covering = self.fetch_oi_buildup('Short Covering').get(symbol) if include_exit_data else None
            
            # Entry decision
            entry_ok, entry_reason = self.check_entry_signal(symbol, pcr, long_buildup)
            
            # Exit decision
            if include_exit_data:
                exit_ok, exit_reason = self.check_exit_signal(symbol, pcr, long_buildup, short_covering)
            else:
                exit_ok, exit_reason = False, 'Exit sentiment not requested'

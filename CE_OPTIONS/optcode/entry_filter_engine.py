    ...
    def validate(self, signal: Dict[str, Any], candle_data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if self.macd_confirmation and isinstance(macd_15m, dict) and 'macd' in macd_15m:
            ...
            if action == 'BUY' and entry_type != 'MOMENTUM' and macd_value < 0:
                ...
                # (Dormant here — the CE bot processes BUY only — but kept in sync with PUT_OPTIONS.)
...
class ComprehensiveEntryFilter:
    ...
    def validate(self, signal: Dict[str, Any], market_data: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        if self.require_all_filters:
            ...
            # Data-aware threshold. "Evaluable" = validators that produced a real verdict (a counted
            # pass or a hard fail); the ones in skipped_validators had no data (throttled broker fetch)
            # or weren't applicable to this entry type. We must not demand more passes than evaluable —
            # otherwise a rate-limit-starved fetch at the open guarantees a reject even on a clean
            # breakout. But we DO require a coverage floor so we never fire on near-zero validation.

...
class OptionsSignalValidator:
    ...
    def _min_quality_for_entry_type(entry_type: str) -> float:
        ...
        if normalized == 'PRE_BREAKOUT':
            return OptionsTradingConfig.MIN_CONFIDENCE_PRE_BREAKOUT
        ...
        if normalized == 'DEEP_MACD_REVERSAL':
            return OptionsTradingConfig.MIN_CONFIDENCE_DEEP_MACD_REVERSAL
        if normalized == 'MOMENTUM_ACCELERATION':
            return OptionsTradingConfig.MIN_CONFIDENCE_MOMENTUM_ACCELERATION
    ...
    def _is_reentry_setup(entry_type: str, setup_sequence: Optional[int], tv_trigger_flag: str = '') -> bool:
        ...
        normalized = str(entry_type or '').upper().strip()
        if normalized not in {'PULLBACK', 'PRE_BREAKOUT'}:
            return False
    ...
    def validate_options_signal(alert: Dict[str, Any]) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            day_change = float(alert.get('day_change', 0) or 0)
            try:
            ...
            reentry_allowed = os.getenv('CALL_OPTIONS_REENTRY_ALLOWED', 'false').lower() == 'true'
            ...
            if is_reentry_setup and not reentry_allowed:
                message = (
                    f"CALL re-entry disabled by CALL_OPTIONS_REENTRY_ALLOWED | "
                    f"entry_type={entry_type} | setup_sequence={setup_sequence}"
                )
                logger.warning(f"SIGNAL_VALIDATE: REJECTED | {message} | symbol={symbol}")
                log_signal_validation(symbol, False, message, action=action, underlying=underlying)
                return False, message, None

            if is_reentry_setup:
                previous_alert = OptionsSignalValidator._get_previous_alert(underlying, entry_type)
                min_reentry_price_step_pct = float(os.getenv('CALL_OPTIONS_REENTRY_MIN_ALERT_STEP_PCT', '1.0'))
                if alert_price <= 0:
                    message = f"CALL re-entry requires valid alert price | entry_type={entry_type}"
                    logger.warning(f"SIGNAL_VALIDATE: REJECTED | {message} | symbol={symbol}")
                    log_signal_validation(symbol, False, message, action=action, underlying=underlying)
                    return False, message, None
                if not previous_alert or float(previous_alert.get('alert_price', 0) or 0) <= 0:
                    message = (
                        f"CALL re-entry missing prior alert baseline | entry_type={entry_type} | "
                ...
                min_required_price = previous_alert_price * (1 + min_reentry_price_step_pct / 100.0)
                if alert_price < min_required_price:
                    message = (
                        f"CALL re-entry alert price ₹{alert_price:.2f} < prior alert ₹{previous_alert_price:.2f} "
            ...
            if action == 'BUY' and alert_price > 0:

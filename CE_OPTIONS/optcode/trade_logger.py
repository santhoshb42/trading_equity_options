...
from .optconfig import BASE_DIR, DATA_DIR, BOT_MODE
...
class TradeLogger:
    ...
    def __init__(self, base_path: str = None):
        if base_path is None:
            # Use mode-isolated data dir from optconfig
            self.data_dir = DATA_DIR
            self.logs_dir = BASE_DIR / BOT_MODE / "logs"
        ...
        self.csv_headers = [
            ...
            'actual_outcome',
            'ml_accuracy',
            # ── Technical snapshot at entry (logged for post-trade analysis) ──
            # TradingView values (from webhook signal)
            'tv_rsi',           # RSI from TradingView alert
            'tv_adx',           # ADX from TradingView alert
            'tv_vwap_pct',      # VWAP distance % (positive = above VWAP)
            'tv_macd_hist',     # MACD histogram from TradingView
            'tv_ema_spread',    # EMA fast-slow spread %
            'tv_vol_ratio',     # Volume / MA20 ratio
            'tv_day_change',    # Underlying day change %
            'tv_market_trend',  # GOOD / NEUTRAL / BAD
            # Computed from live 1-min candles at entry
            'calc_rsi',         # RSI(14) computed from 1-min candles
            'calc_ema9_up3',    # EMA9 rising for 3 consecutive bars (True/False)
            'calc_ema20_up3',   # EMA20 rising for 3 consecutive bars (True/False)
            'calc_ema_aligned', # price > EMA9 > EMA20 (True/False)
            'calc_di_spread',   # +DI minus -DI (positive = bullish directional)
            'calc_bb_pos',      # BB position: (close-lower)/(upper-lower); >1 = above upper band
            'calc_day_pct',     # Price position in today's range: 0=day low, 1=day high
            'calc_or15_bo',     # Above 15-min opening range high (True/False)
            # Time context
            'entry_min_from_open',  # Minutes since 09:15
            'entry_session',        # MORNING / MIDDAY / AFTERNOON
        ]
    ...
    def log_trade_entry(self,
                       ...
                       tech_snapshot: Optional[Dict] = None) -> str:
        """
        Log trade entry.
        ...
        with self.lock:
            ...
            trade_data = {
                ...
                'ml_accuracy': '',
                # ── Technical snapshot ────────────────────────────────────────
                'tv_rsi':             ts.get('tv_rsi', ''),
                'tv_adx':             ts.get('tv_adx', ''),
                'tv_vwap_pct':        ts.get('tv_vwap_pct', ''),
                'tv_macd_hist':       ts.get('tv_macd_hist', ''),
                'tv_ema_spread':      ts.get('tv_ema_spread', ''),
                'tv_vol_ratio':       ts.get('tv_vol_ratio', ''),
                'tv_day_change':      ts.get('tv_day_change', ''),
                'tv_market_trend':    ts.get('tv_market_trend', ''),
                'calc_rsi':           ts.get('calc_rsi', ''),
                'calc_ema9_up3':      ts.get('calc_ema9_up3', ''),
                'calc_ema20_up3':     ts.get('calc_ema20_up3', ''),
                'calc_ema_aligned':   ts.get('calc_ema_aligned', ''),
                'calc_di_spread':     ts.get('calc_di_spread', ''),
                'calc_bb_pos':        ts.get('calc_bb_pos', ''),
                'calc_day_pct':       ts.get('calc_day_pct', ''),
                'calc_or15_bo':       ts.get('calc_or15_bo', ''),
                'entry_min_from_open':ts.get('entry_min_from_open', ''),
                'entry_session':      ts.get('entry_session', ''),
            }
...
def get_trade_logger(base_path: str = None) -> TradeLogger:

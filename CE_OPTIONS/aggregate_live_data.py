"""
Aggregates OTM and ITM live data into a single CE_OPTIONS-level view.
Reads from OTM/data/ and ITM/data/, writes CE_OPTIONS/live_data.json
and CE_OPTIONS/live_data_trades.csv with mode column added.
...
def _read_csv_trades(path: Path, mode_label: str) -> tuple[list, list]:
...
def _merge_csv() -> str:
    ...
    header = (
        f"CE_OPTIONS Aggregated | Last Updated: {now}\n"
...
def start_aggregator(interval: int = 10) -> threading.Thread:
    ...
    t = threading.Thread(target=_loop, args=(interval,), daemon=True, name="ce-aggregator")
    t.start()
    return t

...
_arg_parser.add_argument('--mode', choices=['OTM', 'ITM'], default='OTM',
                         help='Bot mode: OTM (port 8081) or ITM (port 8080)')
...
# Aggregated live data (merges OTM+ITM into CE_OPTIONS/live_data.json)
try:
...
class OptionsTradingBot:
    ...
    def initialize(self):
        ...
        # Start combined live data aggregator (merges OTM+ITM → CE_OPTIONS/live_data.json)
        if HAS_AGGREGATOR:
            start_aggregator(interval=10)
            logger.info("BOT_INIT: CE_AGGREGATOR_STARTED")
...
if __name__ == "__main__":
    ...
    def is_our_bot_running(pid: int) -> bool:
        """Return True only if PID exists AND its cmdline belongs to CE_OPTIONS/main.py."""
        try:
            os.kill(pid, 0)
        except OSError:
        ...
        try:
            ...
            return 'CE_OPTIONS/main.py' in cmdline or 'CE_OPTIONS/main.py' in cmdline
        except Exception:

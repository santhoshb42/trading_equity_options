...
# =============================================================================
# ML Data Mapping Utilities
# =============================================================================

def extract_underlying_from_symbol(symbol: str) -> str:
    """
    Extract underlying asset name from full option contract symbol.
    
    This is CRITICAL for ML data persistence across contract expirations.
    After 30 days, contracts expire and symbol changes, but underlying remains same.
    
    Examples:
    - BANKNIFTY26DEC24000CE → BANKNIFTY
    - NIFTY02JAN2524000CE → NIFTY  
    - FINNIFTY30JAN25000CE → FINNIFTY
    - RELIANCE27DEC24150PE → RELIANCE
    - TCS02JAN2510000CE → TCS
    
    Algorithm:
    1. Find where date portion starts (pattern: YYMONTHDD or YYMMDD)
    2. Everything before date = underlying
    3. Handle both formats:
       - Format 1: UNDERLYING + YY + MONTH(3 chars) + DD + STRIKE + (CE|PE)
         Example: BANKNIFTY25DEC1900CE
       - Format 2: UNDERLYING + YYMMDD + STRIKE + (CE|PE)
         Example: POWERINDIA251222500CE
    
    Args:
        symbol: Full option contract symbol (string)
    
    Returns:
        Underlying asset name (string)
        Returns full symbol if parsing fails (fallback)
    
    Data Integrity Note:
    ⚠️ CRITICAL: ML learning data MUST be indexed by underlying, not full symbol!
    If indexed by full symbol:
    - BANKNIFTY26DEC24 data (3 trades, 2 wins) = LOST after expiry
    - New contract BANKNIFTY02JAN25 starts with 0 trades
    - Learning completely reset monthly
    
    If indexed by underlying:
    - BANKNIFTY data accumulates: 6 trades total, 4 wins, 66% win rate PERSISTS
    - Data never lost, learning improves continuously
    """
    try:
        # Known index/stock underlyings - check these first (most common)
        common_underlyings = {
            'BANKNIFTY', 'NIFTY', 'FINNIFTY', 'RELIANCE', 'TCS', 'INFY',
            'HDFC', 'ICICIBANK', 'SBIN', 'TATASTEEL', 'HCLTECH', 'WIPRO',
            'MARUTI', 'BAJAJ', 'APOLLOHOSP', 'BPCL', 'COALINDIA', 'ONGC',
            'JSWSTEEL', 'POWERGRID', 'BHARTIARTL', 'ADANIPORTS', 'AXISBANK',
            'BAJAJFINSV', 'SUNPHARMA', 'DRREDDY', 'ASIANPAINT', 'ITC',
            'EICHERMOT', 'GAIL', 'NTPC', 'CIPLA', 'DIVISLAB', 'TECHM',
            'AUTOPSY', 'GOOGLEL', 'POWERINDIA', 'LT'
        }
        
        # Check if any known underlying is at start of symbol
        for underlying in sorted(common_underlyings, key=len, reverse=True):
            if symbol.startswith(underlying):
                return underlying
        
        # If not found in common list, use regex parsing
        # Format 1: UNDERLYING + YY + MONTH(3 chars) + STRIKE + (CE|PE)
        # Example: BANKNIFTY25DEC1900CE
        match = re.match(r'^([A-Z]+?)(\d{2})([A-Z]{3})(\d+)(CE|PE)$', symbol)
        if match:
            return match.group(1)  # underlying is first capture group
        
        # Format 2: UNDERLYING + YYMMDD + STRIKE + (CE|PE)
        # Example: POWERINDIA251222500CE
        match = re.match(r'^([A-Z]+?)(\d{6})(\d+)(CE|PE)$', symbol)
        if match:
            return match.group(1)  # underlying is first capture group
        
        # Fallback: Return original symbol if can't parse
        logger.warning(f"⚠️ Could not parse underlying from symbol: {symbol}")
        return symbol
        
    except Exception as e:
        logger.error(f"❌ Error extracting underlying from {symbol}: {str(e)}")
        return symbol

...
CRON_JOB="30 15 * * 1-5 /root/santhosh/trading/options/run_eod_learning.sh"
...
if grep -q "run_eod_learning.sh" "$TEMP_CRON"; then
    echo "⚠️  EOD Learning cron job already exists"
    echo "Current entry:"
    grep "run_eod_learning.sh" "$TEMP_CRON"
    rm "$TEMP_CRON"
    exit 0
fi
...
echo "  Script: /root/santhosh/trading/options/run_eod_learning.sh"
echo "  Log: /root/santhosh/trading/options/logs/eod_learning.log"
echo ""

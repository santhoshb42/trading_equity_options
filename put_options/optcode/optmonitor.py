...
class OptionPosition:
    ...
    def update_market_data(self, current_premium: float, greeks: Dict[str, float], iv: float):
        ...
        IMPORTANT FOR PE (PUT) OPTIONS:
        - CE (BUY): highest_premium = highest profit point (premium increases)
        - PE (SELL): lowest_premium = highest profit point (premium decreases)
        
        The current logic tracks highest_premium which works for CE.
        For PE, we need to consider that profit occurs when premium DECREASES.
        """
        self.current_premium = current_premium
        
        # Track highest premium reached (for trailing exit on CE options)
...
class OptionPositionMonitor:
    ...
    def check_trailing_stop_losses(self) -> List[Dict[str, Any]]:
        ...
        for symbol, position in positions:
            ...
            # - highest_premium = best profit point
            # - peak_gain_percent = (highest - entry) / entry * 100
            # - Positive % = profit (e.g., 27.1% gain = GOOD)
            #
            # PE (PUT/SELL):
            # - Profit when premium DECREASES  
            # - Lowest_premium = best profit point
            # - peak_gain_percent should be (entry - lowest) / entry * 100
            # - But current code calculates (highest - entry) which tracks WORST point
            #
            # TEMPORARY: The bot will use this logic but track worst-case (highest premium)
            # This means TRIAL SL activates when losses are worst, not profits best
            # TODO: Implement lowest_premium tracking for PE
    ...
    def check_stale_consolidation_exits(self) -> List[Dict[str, Any]]:
        ...
        for symbol, position in positions:
            ...
            # 2. TRIAL_SL not activated (peak never hit threshold%) - signal trend died early
            # 3. Either: (a) Near break-even/profit, OR (b) Started losing but peak was minimal
            
            # 🔧 KEY FIX: Use position's pre-calculated threshold, not just checking trial_sl_enabled
            # If peak < expected_threshold, TRIAL_SL won't activate, so check STALE
            peak_vs_threshold = peak_profit_pct >= (position.trial_sl_expected_threshold / 100.0 if position.trial_sl_expected_threshold else 0.10)
            
            if (hold_time_min >= stale_hold_time_min and  # Been stale for 15+ mins
                not position.trial_sl_enabled and  # Not yet activated
                not peak_vs_threshold):  # Peak < expected threshold (won't activate on its own)
                ...
                if should_exit:
                    logger.warning(f"STALE_CONSOLIDATION_TRIGGERED: {symbol} | Hold: {hold_time_min:.1f}min | "
                                 f"Trial_SL: {position.trial_sl_enabled} | Peak: +{peak_profit_pct*100:.2f}% (< 10%) | "
                                 f"Current: {current_pnl_pct*100:.2f}% | "
                                 f"Action: Exit stale position before momentum reversal")
                    ...
                    if pnl:
                        ...
                        logger.warning(
                            ...
                            f"PnL: ₹{pnl['pnl']:.2f} | "
                            f"Prevented momentum reversal loss (peak < 10%, TRIAL_SL inactive)"
                        )
